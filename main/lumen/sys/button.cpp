...
void power_button_handler(void* parameters)
{
    ...
    led_ring_color color_ring_head = red_ring;
    ...
    while (true) {
        ...
        while (xTaskGetTickCount() < end_debounce) {
            ...
            if (xTaskGetTickCount() < end_debounce) {
                xSemaphoreTake(power_button_semaphore,
                               end_debounce - xTaskGetTickCount());
            }
        }

        ESP_LOGI(tag, "Pressed %d times", count);

        if (count == 1) {
            color_ring_head =
                static_cast<led_ring_color>((scoped_color + 1) % 3);
        } else {
            color_ring_head =
                static_cast<led_ring_color>((scoped_color + 2) % 3);
        }
...
led_ring_color current_color() { return scoped_color; }
